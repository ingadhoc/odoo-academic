    ...
    def _default_portal(self):
        ...
        if active_model == "res.partner" and active_id:
            ...
            if partner_type == "gral_administrator":
                groups = self.env.ref("academic.group_portal_gral_administrator")
            elif partner_type == "administrator":
                groups = self.env.ref("academic.group_portal_administrator")
            elif partner_type == "teacher":
                groups = self.env.ref("academic.group_portal_teacher")
            elif partner_type == "parent":
                groups = self.env.ref("academic.group_portal_parent")
            elif partner_type == "student":
                groups = self.env.ref("academic.group_portal_student")
            return groups.id
