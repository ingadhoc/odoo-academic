...
class AcademicEvaluationReport(models.Model):
    ...
    input_avg_score = fields.Float(
        'Avg Score',
        readonly=True,
        group_operator='avg',
    )
    input_min_score = fields.Float(
        'Min Score',
        readonly=True,
        group_operator='min',
    )
    input_max_score = fields.Float(
        'Max Score',
        readonly=True,
        group_operator='max',
    )
    ...
    def init(self):
        tools.drop_view_if_exists(self._cr, 'academic_evaluation_report')
        self._cr.execute("""
...
SELECT
    ...
    INNER JOIN survey_user_input
        on survey_user_input_question_score.user_input_id =
         survey_user_input.id
