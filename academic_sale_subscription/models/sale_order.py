...
class SaleOrder(models.Model):
    ...
    def action_update_prices(self):
        if self._context.get('action_update_subscription_prices'):
            ...
            definido en recurring prices, otra forma de ingresar es con el approach de sale_subscription_ux
            use_time_based_rules y que haya algún precio para actualizar mayor a 0, si es 0 entonces con loyalty
            termina dando error.
            ...
            '''
            if lines_no_recurring_pricing != self.order_line or (
                self.pricelist_id and not self.pricelist_id.use_time_based_rules and
                any(line._get_display_price() > 0.0 for line in self.order_line)
            ):
    ...
    def get_duplicate_subscription_ids(self):
        ...
        subscriptions = self.search(
            [('partner_id', 'in', list(partner_ids)), ('order_line', '!=', False), ('subscription_state', '=', '3_progress')]
        )
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        param_enabled = self.env['ir.config_parameter'].sudo().get_param(
            'academic_sale_subscription.enable_zero_price_subscription_invoice'
        )
        if not self or not param_enabled:
            return base_lines

        extra_lines = self.env['sale.order.line'].search([
            ('order_id', 'in', self.ids),
            ('id', 'not in', base_lines.ids),
            ('state', '=', 'sale'),
            ('display_type', '=', False),
