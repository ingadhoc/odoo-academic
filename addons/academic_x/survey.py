	...
	def autom_add_groups(self, cr, uid, ids, context=None):
		...
		for survey in self.read(cr, uid, ids, ['level_ids', 'subject_ids'], context=context):
			...
			if not level_ids or not subject_ids:
				raise osv.except_osv(_('Atuomatically Add Groups Error!'), _('To auotmatically add Groups you should set level and subject!'))
			group_ids = self.pool['academic.group'].search(cr, uid, [('level_id','in',level_ids),('subject_id','in',subject_ids)], context=context)
