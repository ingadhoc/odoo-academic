...
class survey_page(osv.Model):
    ...
    def get_user_input(self, cr, uid, ids, token, context=None):
        ...
        user_input_ids = user_input_obj.search(cr, uid, [('token','=',token)], context=context)
        if user_input_ids:
...
class survey_survey(osv.Model):
    ...
    def set_invisible(self, cr, uid, ids, context=None):
        ...
        group_evaluation_ids = group_evaluation_obj.search(cr, uid, [('survey_id', 'in', ids)], context=context)
    ...
    def autom_add_groups(self, cr, uid, ids, context=None):
        new_record_ids = []
        for survey in self.read(cr, uid, ids, ['level_ids', 'subject_ids','group_year'], context=context):
            ...
            if not level_ids or not subject_ids:
                raise osv.except_osv(_('Automatically Add Groups Error!'), _('To automatically add Groups you should set level, subject and year!'))
            group_ids = self.pool['academic.group'].search(cr, uid, [('level_id','in',level_ids),('subject_id','in',subject_ids),('year','=',group_year)], context=context)
            survey = self.browse(cr, uid, survey_id, context=context)
            actual_group_ids = [x.group_id.id for x in survey.group_evaluation_ids]
